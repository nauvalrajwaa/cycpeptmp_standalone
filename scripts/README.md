...
## WSL / Windows MOE usage

If MOE is installed on Windows and you're running the pipeline from WSL, you can point `predict.py` at the Windows `moebatch.exe` in a few ways:

- Pass a WSL-accessible Windows `moebatch` path (WSL path):

```bash
python scripts/predict.py --file sequences.txt --moebatch "/mnt/c/Program Files/MOE/moebatch.exe"
```

- Or pass a Windows path directly (the script will attempt to convert it to a `/mnt/` path):

```bash
python scripts/predict.py --file sequences.txt --moebatch "C:\\Program Files\\MOE\\moebatch.exe"
```

- Or run `generate_moe.py` explicitly with `moebatch` and then run `predict.py` using `--moe-dir`:

```bash
python scripts/generate_moe.py --sequences-file sequences.txt --out-dir out/moe_csvs --moebatch "/mnt/c/Program Files/MOE/moebatch.exe"
python scripts/predict.py --resume runs/run_xxx --moe-dir out/moe_csvs
```

Notes:
- You can also use `--dry-run` on `generate_moe.py` to print the commands that would be executed.
- If you prefer not to run MOE from WSL, generate MOE CSVs on Windows and pass the folder via `--moe-dir` to `predict.py`.
